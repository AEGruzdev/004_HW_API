...
class SaveToYD:
    ...
    #Внутренний метод, создает словарь, состоящий только из тех пород, которые имеют подпороды
    def _create_dogs_dict(self):
    ...
    #Внутренний метод, создает словарь, где ключи - породы и подпороды, а значения - url
    def _create_dogs_urls(self):
        url_breeds = 'https://dog.ceo/api/breeds/list/all'
    ...
    #Загрузка фотографий, if - породы, не имеющий под-пород, для них грузим все фото, else - под-породы, для них по 1 случайной фотке
    def photos_one_breed(self, breed_dog, folder_name):
        if breed_dog not in self._create_dogs_dict():
            url_photos_breed = self._create_dogs_urls()[breed_dog]
            ...
            for elem in tqdm(photo_url):
                ...
                requests.put(f'{url_for_downloads.json()['href']}', data = photo_dog_url)
                if self.dict_photo.get(elem.split('/')[-2]) is None:
            ...
            for elems in tqdm(self._create_dogs_dict()[breed_dog]):
                sub_breed = f'{breed_dog}-{elems}'
                url_photos_breed = f'{self._create_dogs_urls()[sub_breed]}/random'
                breed_photo = requests.get(url_photos_breed)
                photo_url = breed_photo.json()['message']
                ...
                headers_write = {'Authorization': f'OAuth {self.token}'}
                url_for_downloads = requests.get(url_for_get_downloads, params=params_write_file, headers=headers_write)
                requests.put(url_for_downloads.json()['href'], data=photo_dog_url)
                if self.dict_photo.get(photo_url.split("/")[-2]) is None:  
...
dogs.photos_one_breed('hound', 'DOGS')